...
from prompt import THOUGHT_REWRITE_PROMPT_EN, ROLE_PLAY_PROMPT_EN, CHARACTER_SETTINGS_EN
from config import MODEL_request_by_API_async
import os

# python code/data_gen/distill_async_s1k.py
async def process_single_example(data, engine, gen_param, role=None, is_local=False):
    try:
        if role:
            ...
            message = ROLE_PLAY_PROMPT_EN.format(
                character_setting=character_setting,
                question=data['question'].strip(),
                reasoning=data['deepseek_thinking_trajectory'].strip(),
                answer=data['deepseek_attempt'].strip(),
                ground_truth=data['solution'].strip()
            )
        else:
            message = THOUGHT_REWRITE_PROMPT_EN.format(
                question=data['question'].strip(),
                reasoning=data['deepseek_thinking_trajectory'].strip(),
                answer=data['deepseek_attempt'].strip(),
                ground_truth=data['solution'].strip()
        ...
        if response:
            if not role:
                ...
                answer_match = re.search(r'###\s*[Aa]nswer\s*:(.+?)(?=###|$)', response, re.DOTALL)
            else:
            ...
            if thought_match and answer_match:
                ...
                tqdm.write(f"Failed to extract thought content from response: {response}...")
        else:
...
async def process_file_async(engine, input_file, output_file, batch_size=5, is_local=False, role=None):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        with tqdm(total=total_lines, desc="Processing examples", ncols=100) as pbar:
            ...
            if current_batch:
                results = await process_batch(current_batch, engine, gen_param, role)
                processed_data.extend(results)
                pbar.update(len(current_batch))
...
def main():
    parser = argparse.ArgumentParser(description='Process files with async batch processing.')
    parser.add_argument('--model', type=str, default="Qwen/Qwen2.5-72B-Instruct", help='model name')
    parser.add_argument('--input_file', type=str, default="datasets/raw/s1k.jsonl", help='input file')
    parser.add_argument('--output_file', type=str, default="datasets/s1k.jsonl", help='output file')
